...
void CMenuWnd::Notify(TNotifyUI& msg)
{
    if (msg.sType == _T("itemclick")) {
        if ((msg.pSender->GetName() == L"menu_official")
            || (msg.pSender->GetName() == L"menu_feedback")
            || (msg.pSender->GetName() == L"menu_help")) {
            ::ShellExecute(NULL, _T("open"), _T("https://github.com/duilib/duilib"), NULL, NULL, SW_SHOWNORMAL);
        } else if (msg.pSender->GetName() == L"menu_open") {
            ::SendMessage(m_hParent, WM_SHOWWINDOW, 1, 0);
        } else if (msg.pSender->GetName() == L"menu_mini2tip") {
            ::SendMessage(m_hParent, WM_SHOWWINDOW, 0, 0);
        } else if (msg.pSender->GetName() == L"menu_exit") {
