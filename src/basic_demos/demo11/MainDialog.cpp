...
void CMainDialog::OnKillFocus( CControlUI* pSender )
{
	if (pSender->GetName() == _T ("editSearch")) {
		...
		if (pbtnTip && peditSearch) {
			...
			if (strText.IsEmpty()) {
				strText = L"請輸入查找條件";
				pbtnTip->SetAttribute(L"textcolor", L"#00A0A0A4");
			} else {
				pbtnTip->SetAttribute(L"textcolor", L"#FF000000");
			}
...
void CMainDialog::OnNotify( TNotifyUI& msg )
{
	if (msg.sType == _T ("itemactivate")) {
		if (_tcsicmp (msg.pSender->GetClass(), L"ListUI") == 0) {
			CListUI* pList = static_cast <CListUI*> (m_pm.FindControl(_T ("lstNormal")));
			ASSERT(pList);
			if (pList) {
				CDuiString str;
			...
			_tcsicmp (msg.pSender->GetParent ()->GetParent ()->GetClass(), L"Tree") == 0)
		{
	...
	if (wcsicmp (msg.pSender->GetClass(), _T ("ListHeaderItemUI")) == 0 && msg.sType == _T ("headerclick")) {
		CListUI* pList = static_cast <CListUI*> (m_pm.FindControl(_T ("lstNormal")));
		ASSERT(pList);
		if (pList) {
...
{
	CComboUI* pcbx2 = static_cast <CComboUI*> (m_pm.FindControl(L"combo2"));
	assert (pcbx2);
	if (pcbx2) {
