...
public:
	...
	{
		CActiveXUI *pActiveXUI = static_cast<CActiveXUI*>(m_pm.FindControl(_T("ActiveXDemo1")));
		if (pActiveXUI)
		{
		...
		// 改变进度条的位置
		CProgressUI *pProgressUI = static_cast<CProgressUI*>(m_pm.FindControl(_T("ProgressDemo1")));
		if (pProgressUI)
		{
		...
		// 改变滑动条的位置
		CSliderUI *pSliderUI = static_cast<CSliderUI*>(m_pm.FindControl(_T("SliderDemo1")));
		if (pSliderUI)
		{
		...
		CDuiString str;
		CListUI *pListUI = static_cast<CListUI*>(m_pm.FindControl(_T("ListDemo1")));
		if (pListUI)
		{
			for (int i = 0; i < 100; ++i)
			{
	...
	virtual void Notify(TNotifyUI& msg)
	{
		if (msg.sType == _T("selectchanged"))
		{
			...
			CTabLayoutUI *pTabLayoutUI = static_cast<CTabLayoutUI*>(m_pm.FindControl(_T("tabTest")));
			if (pTabLayoutUI)
			{
